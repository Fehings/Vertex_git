...
if sum(inGroup) ~= 0
    ...
    for iSynType = 1:size(SynapseModel, 2)
        RecVar.apre_synRecording(inGroup, iSynType, recTimeCounter,:) = ...
            SynapseModel{iGroup, iSynType}.Apost(RecVar.apre_synRecCellIDArr(inGroup, 1));
        if max(SynapseModel{iGroup, iSynType}.Apost(RecVar.apre_synRecCellIDArr(inGroup, 1)))>0
            disp(SynapseModel{iGroup, iSynType}.Apost(RecVar.apre_synRecCellIDArr(inGroup, 1)));
        end
    end
end
