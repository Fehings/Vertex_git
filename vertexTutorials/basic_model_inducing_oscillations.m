...
noiseScaler = 0.7; % Scaling parameter for the background noise in the model
% tweak the noise scaler? Until it is closer to the threshold between
% oscillations/ no oscillations 

synapticScaler =  0; % Scaling parameter for initial synaptic weights
...
% Using a step current here to trigger seizure like activity

% input to the excitatory population, Input(2), as there is already a
% background noise input defined in the Neuron Params.
NeuronParams(1).Input(2).inputType = 'i_step';
NeuronParams(1).Input(2).amplitude = 50000;                 % Change this and make it bigger/ longer/ more repeats?
NeuronParams(1).Input(2).timeOn = [200]%,280,360];
NeuronParams(1).Input(2).timeOff = [215]%,320,400];
NeuronParams(1).Input(2).compartmentsInput = 1;


% if changing the noise level + the step current input doesn't trigger
% seizures, then, try adding a 'i_step_ou' type thing to allow us to ramp
% up the background noise over time (OR change the VERTEX simulate code to
% allow the background noise to be time dependent (so, pass it a vector of
% noise over time instead of a scalar)
