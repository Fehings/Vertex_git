...
singlePulseDir = [F1000_data_dir filesep 'singlepulse' filesep 'singlepulse' filesep];
nosynapseDir = [F1000_data_dir filesep 'singlepulse' filesep 'no_synapses' filesep];
tmcurrentsDir = [F1000_data_dir filesep 'singlepulse' filesep 'transmembraneCurrents' filesep];
pairedPulseDir = [F1000_data_dir filesep 'pairedpulse' filesep ];
pairedPulseRange = [pairedPulseDir filesep 'pairedpulserange' filesep 'pairedPulse' filesep];
pairedPulseDetailed = [pairedPulseDir filesep 'pairedpulseInDetail' filesep];
pairedPulseWithCurrents = [pairedPulseDir filesep 'transmembraneCurrents' filesep];
tbsDir = [F1000_data_dir filesep 'tbs'];
tbsMultipleRuns = [tbsDir filesep 'multipleruns' filesep];
tbsWithWeights = [tbsDir filesep 'withWeights' filesep];

%% Figure 10 A and C
RecordingSettings.saveDir = [singlePulseDir filesep 'singlepulse_1001'];
...
RecordingSettings.saveDir = [tmcurrentsDir 'singlepulsesynI1001'];
Results = loadResults(RecordingSettings.saveDir,0);
...
RecordingSettings.saveDir = [tmcurrentsDir 'singlepulse_1001'];
Results = loadResults(RecordingSettings.saveDir,0);
plotCellCurrents(Results,1:50,1,[7500 9800]);
%
% (D) Transmembrane current at each compartment and (E) LFP generated by
% each compartment.
plotLFPbycompartment;

%% Figure 12
path = [pairedPulseRange 'pairedpulse_'];
...
plotPPresponseamps;
%% Figure 13 
RecordingSettings.saveDir = [pairedPulseDetailed 'pairedpulse1001'];
Results = loadResults(RecordingSettings.saveDir,1);
figure;
plotPPsynapticresources;
%%
RecordingSettings.saveDir = [pairedPulseDir 'pairedpulsesynI1001'];
Results = loadResults(RecordingSettings.saveDir,1);
NP = Results.params.NeuronParams(14);
NP.labelnames = {'somaID','basalID','obliqueID','apicalID','trunkID','tuftID'};
times = [1500 6000];
...
%%
RecordingSettings.saveDir = [pairedPulseWithCurrents 'pairedpulse1001'];
Results = loadResults(RecordingSettings.saveDir,1);
plotLFPbycompartment;

%% Figure 14
RecordingSettings.saveDir = [tbsDir 'pulse_stdp'];
Results = loadResults(RecordingSettings.saveDir,1);
...
rasterFigureImproved = plotSpikeRaster(Results, rasterParams);
%% Figure 15 B, C, D
RecordingSettings.saveDir = [tbsWithWeights 'tbs_weights1001'];
Results = loadResults(RecordingSettings.saveDir,1);
