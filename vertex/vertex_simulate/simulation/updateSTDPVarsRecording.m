...
if sum(inGroup) ~= 0
    ...
    for i = 1:length(SynapseModel(:,1))
        if isa(SynapseModel{i,synMap{i}(iGroup)},'SynapseModel_g_stdp_delays') || isa(SynapseModel{i,synMap{i}(iGroup)},'SynapseModel_g_stdp')
        ...
        if isa(SynapseModel{iGroup,synMap{iGroup}(i)},'SynapseModel_g_stdp_delays') || isa(SynapseModel{i,synMap{i}(iGroup)},'SynapseModel_g_stdp')
