...
if SS.fu_stimulation
    ...
    for iGroup = 1:TP.numGroups
        ...
        InModel{iGroup}.fusParams.C=NP(iGroup).C;    
        InModel.fusParams(iGroup).fusparams.P_in=ones(size(NP(iGroup).C)).*10^5; % units Pa
        InModel.fusParams(iGroup).fusparams.n_a=1.5769e+06;
        InModel.fusParams(iGroup).fusparams.v=ones(size(NP(iGroup).C));
        InModel.fusParams(iGroup).fusparams.z=ones(size(NP(iGroup).C));
        InModel.fusParams(iGroup).fusparams.tstep=SS.timeStep;
        InModel.fusParams(iGroup).fusparams.dCmVm=zeros(size(NP(iGroup).C)); % initialise with no capacitive displacement current.
    end
    ...
    %Assign initial values for parameters needed in the fusBLS function,
    %called later in the UpdateInput method of the InputModel_i_focusedultrasound

    
% also gonna need Vm and Cm 
end
