...
for simStep = 1:simulationSteps
    ...
    if isfield(TP, 'StimulationField')
        ...
        if current_time > TP.StimulationOn(stimcount) && current_time < TP.StimulationOff(stimcount)
            for iGroup = 1:TP.numGroups % For each neuron population..
                if  ~NeuronModel{iGroup}.incorporate_vext % check if they are set to recieve stimulation
                    stimulationOn(NeuronModel{iGroup});   % if they are not, tell them to start recieving stimulation
                 end
                ...
                elseif isfield(TP,'ClosedLoop') 
                    setVext(NeuronModel{iGroup},NeuronModel{iGroup}.v_extCLON*TP.ClosedLoop);% where ClosedLoop is 1 or 0 depending on whether the threshold is met
                    
                end
                % setVext is set external voltage (to be applied as
                % stimulation). In this case, we will want to add a 'closed
                % loop' version of this which will apply the current
                % stimulation value, which we will then update below.
            ...
            if isfield(TP,'ClosedLoop') % NB remember to have TissueParams.ClosedLoop in the main script
                ...
                % average)
                % If this is > TP.threshold that is defined in the main
                % script, we set the Closed loop value to be either 1 or 0.
                % To be mulitplied by NeuronModel{iGroup}.v_extCLON 
                
                % Access current LFP using: (Frances to figure out why
                % there are 2 cells!)
                % assign first cell to a new variable so we can access the
                % contents
                % currLFP = RecVar.LFPRecording{1}
                % take the mean over the current column,
                % mean(currLFP(:,currtime) < need to find right variable
                % for currtime!
