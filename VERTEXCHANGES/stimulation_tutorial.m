...
%mesh: p,e, and t
%% Stimulation flags, to avoid going all the way through the code to change it:
% Stimulation flags: 1 is electric stimulation, 2 is focused ultrasound. 
% Any other value, or none at all, gives just the default input.
...
for i = 1:length(NeuronParams)
    % if using external stimulation, apply that here:
    if stimflag==1
        NeuronParams(i).Input(2).inputType = 'i_efield';
    elseif stimflag==2
        NeuronParams(i).Input(2).inputType = 'i_focusedultrasound';
    end
...
SimulationSettings.simulationTime = 10; % originally at 50 in Chris's script
...
elseif stimflag==2
    ...
    SimulationSettings.fu_stimulation = true;
end
...
rasterParams.figureID = 6;
rasterFigureImproved6 = plotSpikeRaster(Results, rasterParams);
