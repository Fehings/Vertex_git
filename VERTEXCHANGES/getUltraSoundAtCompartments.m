function [ ultrasound ] = getUltraSoundAtCompartments(TP, StimParams)
...
ultrasound = cell(TP.numGroups,1); % preassign blank cell?
...
for iGroup = 1:TP.numGroups % for each group
    ...
    for iComp = 1:numcompartments % for each compartment
        ultrasound{iGroup}(iComp,:) = activationfunction([point1.x(iComp,:);point1.z(iComp,:)] ,...
